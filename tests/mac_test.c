...
void mac_test_cycle(){
	...
	SkyMACConfig config;
	config.default_tail_length = randint_i32(3,1000);
	config.default_gap_length = randint_i32(3,2000);
	config.default_window_length = randint_i32(3,2000);
	config.minimum_window_length = randint_i32(3, config.default_window_length);
	config.maximum_window_length = randint_i32(config.default_window_length, 2000);
	config.shift_threshold_ms = randint_i32(3,100000);
	...
	mac->last_belief_update = randint_i32(0, MOD_TIME_MS-1);
	mac->T0_ms = randint_i32(0, MOD_TIME_MS-1);
...
void tst_v1(SkyMAC* mac, SkyMACConfig* config){
	int t0 = randint_i32(0, MOD_TIME_MS-1);
	int peer_w = randint_i32(config->minimum_window_length, config->maximum_window_length);
	...
	for (int i = 0; i < 20; ++i) {
		...
		int t1 = wrap_time_ms(t0 + (randint_i32(0,1000) * cycle) + s);
		...
		int t2 = wrap_time_ms(t0 + (randint_i32(0,2000) * cycle) + s );
		...
		int t3 = wrap_time_ms(t0 + (randint_i32(0,2000) * cycle) + s);
		...
		int t4 = wrap_time_ms(t0 + (randint_i32(0,2000) * cycle) + s);
