...
void test1_round(uint64_t NN, int print_on){
	...
	sky_vc_wipe_to_arq_init_state(handle1->arrayBuffers[0], job.now_ms);
...
static void step_forward(int which, TXRXJob* job){
	...
	if((job->now_ms > 10000) && roll_chance(peer->pl_rate/1000.0) && !sky_vc_send_buffer_is_full(
			peer->handle->arrayBuffers[0])){
		Payload* pl = new_random_payload(target, job->now_ms);
		int push_ret = sky_vc_push_packet_to_send(peer->handle->arrayBuffers[0], pl->msg->data, pl->msg->length);
	...
	while ((i+1) < job->inEther->n) {
		...
		if(eframe->rx_ok){
			...
			if(sky_vc_count_readable_rcv_packets(peer->handle->arrayBuffers[0])){
				int seq = -1;
				int red = sky_vc_read_next_received(peer->handle->arrayBuffers[0], tgt, &seq);
				assert(red >= 0);
				assert(seq >= 0);
