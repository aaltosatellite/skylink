...
TEST(create_rings)
{
	...
	// Test creating a sequence that naturally wraps around. (Sequence number higher than max of uint8_t) (300 translates to 44)
...
TEST(wipe_rings)
{
	...
	for (int i = 0; i < 20; i++)
	{
		...
		uint8_t data[10], data2[10];
		for (int j = 0; j < 10; j++)
			data[j] = data2[j] = i;

		// Write the data to the element buffers.
		sky_element_buffer_store(rcv_eb, (const uint8_t*)data, sizeof(data));
		sky_element_buffer_store(send_eb, (const uint8_t*)data2, sizeof(data2));
	}
	...
	sky_element_buffer_destroy(rcv_eb);
	sky_element_buffer_destroy(send_eb);
...
TEST(push_and_read_packets)
{
	...
	SkyElementBuffer *eb = sky_element_buffer_create(/* usable_element_size */ 16, /* element_count */ 20);
	...
	ret = rcvRing_push_rx_packet(rcv_ring, eb, (const uint8_t*)payload, sizeof(payload), 0);
	ASSERT(ret >= 0, "rcvRing_push_rx_packet() returned %d", ret);
	...
	{
		uint8_t readout[120];
		int readout_len = rcvRing_read_next_received(rcv_ring, eb, readout, sizeof(readout));
		ASSERT(readout_len == 120, "readout_len = %d", readout_len);
		ASSERT_MEMORY(payload, readout, readout_len);
	}
	...
	sendRing_push_packet_to_send(send_ring, eb, (const uint8_t*)payload, sizeof(payload));
...
/*
 * Test wrapping around the ring indexes and naturally wrapping around the sequence number with uint8_t overflow.
 */
TEST(wrap_around)
{
	...
	SkySendRing *send_ring = sky_send_ring_create(/* length */ 8, /* initial_sequence */ 250);
	...
	// Create an element buffer for both rings and fill it with some data. (Element usable space is 16 bytes.)
	SkyElementBuffer *eb = sky_element_buffer_create(/* usable_element_size */ 16, /* element_count */ 200);
	...
	uint8_t payload[8 * block_len];
	fillrand(payload, sizeof(payload));

	// Push the packets to the rings.
	for (int i = 0; i < 7; i++)
	{
		printf("Pushing packet #%d\n", i);

		// Push received data block to rcv ring
		int ret = rcvRing_push_rx_packet(rcv_ring, eb, &payload[block_len * i], block_len, 250 + i);
		ASSERT(ret >= 0, "rcvRing_push_rx_packet() returned %d", i, ret);

		// Push
		ret = sendRing_push_packet_to_send(send_ring, eb, &payload[block_len * i], block_len);
		ASSERT(ret >= 0, "sendRing_push_packet_to_send() returned %d", i, ret);
	}

	// Check that the packets were pushed correctly.
	int j = 0;
	sky_arq_sequence_t seq = 250;
	for (uint8_t i = 5; i != 4; i++, j++)
	{
		printf("i = %d\n", i);
...
TEST(resend_list)
{
	...
	// Add some 8 packets to the ring and have some of them be lost.
	uint8_t payload[64];
	fillrand(payload, sizeof(payload));
	for (int i = 0; i < 8; i++)
		sky_vc_push_packet_to_send(vc, (const uint8_t *)payload, sizeof(payload));
...
TEST(lost_packets)
{
	...
	for (int i = 0; i < 8; i++)
	{
		...
		// Push a packet to send ring.
		uint8_t payload[64];
		fillrand(payload, sizeof(payload));
		sky_vc_push_rx_packet(vc, (const uint8_t*)payload, sizeof(payload), i, 10);
	}
	...
	// Push a third packet to send ring.
	{
		uint8_t payload[64];
		fillrand(payload, sizeof(payload));
		sky_vc_push_rx_packet(vc, (const uint8_t*)payload, sizeof(payload), 3, 10);
	}
	...
	// Push sixth packet to send ring.
	{
		uint8_t payload[64];
		fillrand(payload, sizeof(payload));
		sky_vc_push_rx_packet(vc, (const uint8_t*)payload, sizeof(payload), 6, 10);
	}
...
TEST(continuous_pushing)
{
	...
	for (int i = 0; i < n; i++)
	{
		...
		// Push the packet to rcv ring.
		int recieve_ret = sky_vc_push_rx_packet(vc, (const uint8_t*)payload, sizeof(payload), i, 10);

		// Push the packet to send ring.
		int send_ret = sky_vc_push_packet_to_send(vc, (const uint8_t*)payload, sizeof(payload));
