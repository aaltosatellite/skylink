...
TEST(mac_create, ticks)
{
	...
	SkyMAC *mac = sky_mac_create(&config.mac);
...
TEST(mac_window_expand){
	// Default config.
	SkyConfig config = default_config;
	// TODO: SkyMACConfig config = default_config.mac;
 ...
 */
TEST(tx_after_carrier_sense, ticks, SKIP)
{
	GET_PARAM(ticks);
	sky_tick(ticks);
	...
	SkyConfig config = default_config;
	config.mac.carrier_sense_ticks = 100;
	SkyHandle handle = sky_create(&config);
	...
	// Add payload to send buffer
	{
		...
		SkyVirtualChannel *vc = handle->virtual_channels[0];
		sendRing_push_packet_to_send(vc->sendRing, vc->elementBuffer, (const uint8_t*)payload, sizeof(payload));
	}

	sky_tick_t now = units_advance_ticks(10);
	int32_t time_to_window = mac_time_to_own_window(mac, now);
	ASSERT(time_to_window == 0, "time_to_window = %d", time_to_window);

	// Set T0 so that test takes less time and it is not needed to wait for next window.
	mac->T0 = ticks + 450;

	// Fifty ticks into the carrier sense fallback threshold, also want to be outside own window.
	for (int i = 0; i < 10; i++)
	//while (time_to_window > mac->config->carrier_sense_ticks - 50 || time_to_window == 0)
	{
		sky_tick_t now = units_advance_ticks(10);
		time_to_window = mac_time_to_own_window(mac, now);
	}
...
TEST(window_adjustments)
{
	...
	while(i < 16 || mac->window_adjust_counter != 0){
		// Send frame.
		if(sendRing_count_packets_to_send(vc->sendRing, 0) == 0)
			sendRing_push_packet_to_send(vc->sendRing, vc->elementBuffer, payload, sizeof(payload));
		sky_tx(handle, &frame);
		now += cycle_length/16;
		sky_tick(now);
		i++;
	...
	sky_send_ring_wipe(vc->sendRing, vc->elementBuffer, 0);
	i = 0;
	// Test shrink.
	while(i < 16 || mac->window_adjust_counter != 0){
