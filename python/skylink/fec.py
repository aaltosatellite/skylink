"""
    Physical layer decoder
"""

import os
import sys
import struct
import hmac
import ctypes
import codecs
from typing import Tuple



class SkylinkError(Exception):
    pass

class FECError(SkylinkError):
    pass

WHITENING = [
	
]


def randomize(raw: bytes) -> bytes:
    """
    Randomized or unrandomized the bytes.

    Args:
        raw: Bytes to be randomized/unrandomized

    Returns:
        Result bytes
    """
    raw = bytearray(raw)
    for i in range(len(raw)):
        raw[i] ^= WHITENING[i]
    return bytes(raw)


try:
    # Try to import
    libskylink = None

    lib_path = os.getenv("SKYLINK_LIB")
    lib_path = "/home/aalto/skylink.git/build/src/libskylink.so"
    libskylink = ctypes.CDLL(lib_path)

    libskylink.encode_rs_8.argtypes = [ctypes.c_char_p, ctypes.c_char_p, ctypes.c_int]
    libskylink.encode_rs_8.restype = None

    libskylink.decode_rs_8.argtypes = [ctypes.c_char_p, ctypes.c_char_p, ctypes.c_int, ctypes.c_int]
    libskylink.decode_rs_8.restype = ctypes.c_int

except:
    print("Warning: Failed to locate 'libskylink.so'")


RS_LENGTH = 32
RS_BLOCK_LENGTH = 255


def decode_rs(data: bytes) -> Tuple[bytes, int]:
    """
    Decode RS(255,223) Error Correction Coding

    Args:
        data: Bytes to be decoded.

    Raises:
        `FECError` in case the decoding fails.

    Returns:
        A tuple containing corrected frame as bytes and number of corrected bit as int.
    """

    if libskylink is None:
        raise RuntimeError("libskylink.so has not been loaded")
    if len(data) <= RS_LENGTH or len(data) > RS_BLOCK_LENGTH:
        raise FECError(f"Invalid length {len(data)}")

    pad = RS_BLOCK_LENGTH - RS_LENGTH - (len(data) - RS_LENGTH)
    data_mutable = ctypes.create_string_buffer(data)
    bits_corrected = libskylink.decode_rs_8(data_mutable, None, 0, int(pad))

    if bits_corrected < 0:
        raise FECError(f"Reed-Solomon decoding error {bits_corrected}")

    return data_mutable[:len(data) - RS_LENGTH], bits_corrected


def encode_rs(data: bytes) -> bytes:
    """
    Encode RS(255,223) Error Correction Coding

    Args:
        data:

    Returns:
        A new bytes containing both the data and error correction trailer.
    """

    if libskylink is None:
        raise RuntimeError("libskylink.so has not been loaded")
    if len(data) > RS_BLOCK_LENGTH - RS_LENGTH:
        raise FECError(f"Invalid length {len(data)}")

    pad = RS_BLOCK_LENGTH - RS_LENGTH - len(data)
    data_mutable = ctypes.create_string_buffer(data, 256)
    libskylink.encode_rs_8(data_mutable, ctypes.cast(ctypes.byref(data_mutable, len(data)), ctypes.POINTER(ctypes.c_char)), pad)

    return data_mutable[0:len(data) + RS_LENGTH]
