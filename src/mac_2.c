...
static int valid_window_length(SkyMACConfig* config, int32_t length){
...
MACSystem* new_mac_system(SkyMACConfig* config){
	...
	set_gap_constant(macSystem, macSystem->gap_constant);
	return macSystem;
}
...
int mac_update_belief(MACSystem* macSystem, SkyMACConfig* config, int32_t now_ms, int32_t peer_mac_length, int32_t peer_mac_remaining){
	if(!valid_window_length(config, peer_mac_length)){
		return -1;
	}
	if(peer_mac_length != macSystem->peer_window_length){
		set_peer_window_length(macSystem, peer_mac_length);
	}
	int32_t implied_t0_for_me = wrap_ms(now_ms + peer_mac_remaining + macSystem->gap_constant, macSystem->cycle_time);
