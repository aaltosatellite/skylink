...
int sky_hmac_extend_with_authentication(SkyHandle self, SendFrame* frame) {
	...
	cf_hmac_update(hmac->ctx, frame->radioFrame.raw, frame->radioFrame.length);
	cf_hmac_finish(hmac->ctx, full_hash);

	//Copy truncated hash to the end of the frame.
	memcpy(&frame->radioFrame.raw[frame->radioFrame.length], full_hash, SKY_HMAC_LENGTH);
...
int sky_hmac_check_authentication(SkyHandle self, RCVFrame* frame) {
	...
	cf_hmac_init(hmac->ctx, &cf_sha256, hmac->key, hmac->key_len);
	cf_hmac_update(hmac->ctx, frame->radioFrame.raw, frame->radioFrame.length - SKY_HMAC_LENGTH);
	cf_hmac_finish(hmac->ctx, calculated_hash);

	//Compare the calculated hash to received one
	uint8_t *frame_hash = &frame->radioFrame.raw[frame->radioFrame.length - SKY_HMAC_LENGTH];
	...
	int32_t jump = wrap_hmac_sequence( (int32_t)(frame->radioFrame.auth_sequence - self->hmac->sequence_rx[frame->radioFrame.vc]));
	...
	self->hmac->sequence_rx[frame->radioFrame.vc] = wrap_hmac_sequence((int32_t)(frame->radioFrame.auth_sequence + 1));
	frame->auth_verified = 1;
	frame->radioFrame.length -= SKY_HMAC_LENGTH;
	return SKY_RET_OK;
}
