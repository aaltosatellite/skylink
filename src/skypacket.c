...
int decode_skylink_packet(SkyRadioFrame* frame){
	...
	frame->hmac_on = (flag & SKY_FLAG_AUTHENTICATED) != 0;
...
int sky_packet_add_extension_arq_rr(SendFrame* frame, uint8_t sequence, uint8_t mask1, uint8_t mask2){
	void* ptr = frame->radioFrame.raw + I_PK_EXTENSIONS + frame->radioFrame.ext_length;
	...
	frame->radioFrame.length = I_PK_EXTENSIONS + frame->radioFrame.ext_length;
	return 1;
}
...
int sky_packet_extend_with_payload(SendFrame* frame, void* pl, int32_t length){
	...
	memcpy(frame->radioFrame.raw + I_PK_EXTENSIONS + frame->radioFrame.ext_length, pl, length);
	frame->radioFrame.length = I_PK_EXTENSIONS + frame->radioFrame.ext_length + length;
	return 0;
}
