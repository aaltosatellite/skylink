...
from cython_skylink import SkyLinkLoop
from cython_skylink import num_virtual_channels
import zmq
import time
import json
...
class SkyModem:
	def __init__(self, receiver_settings, skylink_config, hmac_key_list, vc_port_base):
		self.on = True
	...
	def close(self):
		...
		try:
			...
			pass
		if self.sub_que_process_thread:
			self.sub_que_process_thread.join(timeout=1.0)
		if self.skylink_reception_thread:
	...
	def start(self):
		...
		self.sub_que_process_thread = threading.Thread(target=self.sub_que_process_loop, args=tuple(), daemon=True)
		self.sub_que_process_thread.start()
		self.skylink_reception_thread = threading.Thread(target=self.skylink_reception_loop, args=tuple(), daemon=True)
		self.skylink_reception_thread.start()


	def skylink_reception_loop(self):
		while self.on:
			try:
	...
	def sub_que_process_loop(self):
		while self.on:
			try:
				ID, msg = self.sub_que.get(timeout=0.25)
				DBGPRINT("++ID-msg pulled from sub-queue-{}.".format(ID))
				self.process_frame(ID, msg)
			except Empty:
				pass
			except Exception as e:
